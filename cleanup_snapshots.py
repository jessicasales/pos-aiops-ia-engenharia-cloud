...
python3 cleanup_snapshots.py \
  ...
  --user administrator@vsphere.local \
  --password 'SuaSenha' \
  --exclude-vms vm-banco,vm-erp,vm-oracle \
  --max-age-days 2

Agendamento via cron:
0 2 * * * python3 /opt/scripts/cleanup_snapshots.py --host vcenter.local --user admin@vsphere.local --password 'senha' --exclude-vms vm-banco >> /var/log/snapshot_cleanup.log 2>&1
