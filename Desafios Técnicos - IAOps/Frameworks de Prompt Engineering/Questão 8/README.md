### QUESTÃO 8 - Decisão entre Rollback e Escalamento Emergencial

Como SRE, apoiar a decisão entre fazer rollback do deploy v2.48.0 (subiu ontem) e escalonamento emergencial (aumentar limites do RDS e do pool de conexões) diante de degradação acelerada de latência/erros, pool de conexões exaurido, circuit breaker aberto e backlog crescente na fila do Reator — entregando um post-mortem em markdown.
...
Documento de apoio à decisão (decision-support) em formato post-mortem: aplica semiologia de incidente (leitura cruzada de latência/erros/logs/estado de infra), quantifica a matemática do pool vs. limites do RDS e do HPA, ranqueia hipóteses por confiança e propõe um fluxo de decisão por "gates" com gatilhos objetivos de rollback obrigatório — em vez de escolha binária. A recomendação prioriza rollback (mais rápido, reversível e diagnóstico) condicionada ao Gate 0 (existência de consumidores do endpoint novo), usando escala apenas como contenção quando a quebra de contrato inviabiliza o revert. A estrutura (linha do tempo, matriz de decisão, checklist de evidências) segue o formato padrão dos demais README deste repositório.